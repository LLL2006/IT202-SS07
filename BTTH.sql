...
-- Xác định các sản phẩm có giá cao hơn mặt bằng chung của hệ thống
Select * from san_pham
where price > (select avg(price) from san_pham);

-- Tìm các sản phẩm có số lượng bán ra thấp hơn mức trung bình
select s.*, sum(c.quantity)
from san_pham s
join chi_tiet_don_hang c on s.product_id = c.product_id
group by s.product_id
having sum(c.quantity) < (select avg(total_quantity) from (select sum(quantity) `total_quantity` from chi_tiet_don_hang
group by product_id) a);

-- Xác định khách hàng có tổng chi tiêu cao hơn mức trung bình
select kh.customer_id, kh.customer_name, sum(ctdh.quantity * sp.price) as total_spent 
from khach_hang kh
join don_hang dh on kh.customer_id = dh.customer_id
join chi_tiet_don_hang ctdh on dh.order_id = ctdh.order_id
join san_pham sp on ctdh.product_id = sp.product_id
GROUP BY kh.customer_id, kh.customer_name
having total_spent > (select avg(customer_total) from (select sum(ct.quantity*s.price) as customer_total
    from don_hang d
    join chi_tiet_don_hang ct on d.order_id = ct.order_id
    join san_pham s on ct.product_id = s.product_id
    group by d.customer_id) as avg);